...
let DatabaseService = class DatabaseService {
    ...
    async createInDatabases(anime) {
        const createdAnime = new this.animeModel(anime);
        const animeSaved = await createdAnime.save();
        if (!animeSaved) {
            throw new common_1.InternalServerErrorException('Problema para salvar anime');
        }
        await sonic_1.sonicChannelIngest.push('anime-database', 'animes', `${createdAnime._id}`, `${createdAnime.title}`, {
            lang: 'eng',
        });
        return animeSaved;
    }
