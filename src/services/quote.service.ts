...
import { AnimechanClient } from 'src/client'
...
export class QuoteService {
  ...
  constructor(private prisma: PrismaService, private animechanClient: AnimechanClient) {}
  ...
  getRandomAnimeQuote() {
    return this.animechanClient.getRandomAnimeQuote().pipe(
      ...
      catchException((error) =>
        this.logger.error({ error }, 'Erro ao salvar random quote no banco de dados'),
      ),
    )
  }
  ...
  getAnimesQuoteByTitleInExternalApi(title: string) {
    return this.animechanClient.getAnimesQuoteByTitle(title).pipe(
      ...
      catchException((error) => this.logger.error({ error }, 'Erro ao salvar quotes no banco de dados')),
    )
  }
  ...
  async getAnimesQuoteByTitle(title: string) {
    ...
    if (!quotesAlreadyExist.length) {
      return this.getAnimesQuoteByTitleInExternalApi(title).pipe(
        tap((quotes) => this.logger.info({ quotes }, 'Retornando quotes de API externa')),
        catchException((error) => this.logger.error({ error }, 'Erro ao buscar quotes de API externa')),
      )
    }

    this.logger.info({ quotesAlreadyExist }, 'Retornando quotes a partir do banco de dados')
